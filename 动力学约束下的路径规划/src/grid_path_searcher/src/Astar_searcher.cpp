...
double AstarPathFinder::GetFinalCost()
{          
    return terminatePtr->gScore* resolution;
}
   ...
   for(int i=1; i>=-1; i--)
   {
       for(int j=1; j>=-1; j--)
       {
           for(int k=1; k>=-1; k--)
           {
               ...
               {
                   Vector3i nb_idx;
                   nb_idx(0) = currentPtr->index[0] + i;
                   nb_idx(1) = currentPtr->index[1] + j;
                   nb_idx(2) = currentPtr->index[2] + k;

                   if(isFree(nb_idx)) // excluding obstacles and insuring index is in boundary
                   {
                        double edgeCost = pow(i*i+j*j+k*k, 0.5);
...
//    double hn = 0;
   ...
   double dx = abs(start(0)-end(0));
   double dy = abs(start(1)-end(1));
   double dz = abs(start(2)-end(2));
   //Euclidean
...
//     double hn = dx + dy + dz - (3-sqrt(3))*dmin - (2 - sqrt(2))*(dmid - dmin);
...
{   
   ...
   //Update the startPtr in GridNodeMap
    ...
    while (!openSet.empty()){
        ...
        *
        */
       // set pointer to the node with lowest cost
       currentPtr = openSet.begin()->second;
       // move it from openSet
       currentPtr->id = -1;
