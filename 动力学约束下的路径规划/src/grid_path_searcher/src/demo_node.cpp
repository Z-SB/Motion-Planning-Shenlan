...
AstarPathFinder * Astar_tool     = new AstarPathFinder();
...
{     
    ...
    ROS_INFO("Start position: %f, %f, %f", _start_pt(0), _start_pt(1), _start_pt(2));
    ROS_INFO("Target position: %f, %f, %f", target_pt(0), target_pt(1), target_pt(2));
...
void trajectoryLibrary(const Vector3d start_pt, const Vector3d start_velocity, const Vector3d target_pt)
{
    ...
    double Trajctory_Cost, Astar_Cost, Heuristic_Cost;
    ...
    for(int i=0; i <= _discretize_step; i++){           //acc_input_ax
        ...
        for(int j=0;j <= _discretize_step; j++){        //acc_input_ay
            ...
            for(int k=0; k <= _discretize_step; k++){   //acc_input_az
                ...
                // _max_input_acc = 1; _discretize_step =  2;
                acc_input(0) = double(-_max_input_acc + i * (2 * _max_input_acc / double(_discretize_step)) ); //[-1,0,1]
                acc_input(1) = double(-_max_input_acc + j * (2 * _max_input_acc / double(_discretize_step)) ); //[-1,0,1]
                acc_input(2) = double( k * (2 * _max_input_acc / double(_discretize_step) ) + 0.1);                          //acc_input_az >0.1 [0.1, 1.1,2.1]
                ...
                double delta_time;
                delta_time = _time_interval / double(_time_step); // 1.25/50
                ...
                for(int step=0 ; step<=_time_step ; step ++){
                    ...
                    积分得到在此时的输入acc下，在时间_time_interval 后机器人的位置和速度


                    */
                   pos(0) = pos(0) + vel(0)*delta_time+1/2*acc_input(0)*delta_time*delta_time;  // x1 = x0+v0*t + 1/2*a*t^2
                   pos(1) = pos(1) + vel(1)*delta_time+1/2*acc_input(1)*delta_time*delta_time;  
                   pos(2) = pos(2) + vel(2)*delta_time+1/2*acc_input(2)*delta_time*delta_time;  

                    vel(0) = vel(0) + acc_input(0)*delta_time;  // v1 = v0+a*t
                    ...
                    //check if if the trajectory touch the obstacle
                ...
                /*
                    ...
                    在此时的输入acc下，以时间_time_interval 后机器人的位置和速度为起点，目标点为终点，
                    计算两点之间最优的运动学cost, 并以此路径作为最后最优路径的一部分
                ...
                */
                ...
                //if there is obstacle in the trajectory we need to set 'collision_check = true'
                if(collision)
                {
                    TraLibrary[i][j][k]->setCollisionfree();
                    Heuristic_Cost = Trajctory_Cost;
                }
                else
                {
                    Astar_tool->AstarGraphSearch(pos, target_pt);
                    // 考虑障碍物 不考虑运动学的cost
                    Astar_Cost = Astar_tool->GetFinalCost();

                    Astar_tool->resetUsedGrids();  // reset map
                    Heuristic_Cost = Trajctory_Cost+Astar_Cost;
                }

                ROS_INFO("With input:%f, %f, %f, Heuristic cost: %f", acc_input(0), acc_input(1), acc_input(2), Heuristic_Cost);
                //record the min_cost in the trajectory Library, and this is the part of selecting the best trajectory cloest to the planning traget
                if(Heuristic_Cost<min_Cost && TraLibrary[i][j][k]->collision_check == false){
                    a = i;
                    b = j;
                    c = k;
    ...
    // flag the optimal path in the library
...
int main(int argc, char** argv)
{
    ...
    Astar_tool = new AstarPathFinder();
    Astar_tool -> initGridMap(_resolution, _map_lower, _map_upper, _max_x_id, _max_y_id, _max_z_id);
        ...
        for(int j = 0; j<= _discretize_step;j++){  
            for(int k = 0; k<= _discretize_step;k++){
                if(TraLibrary[i][j][k]->collision_check == false){
                    if(TraLibrary[i][j][k]->optimal_flag == true){ // 绿色为最优路径
                        Line.color.r         = 0.0;
                        Line.color.g         = 1.0;
                        Line.color.b         = 0.0;
                        Line.color.a         = 1.0;
                    }else{  // 蓝色为其他可行路径，但不是最优
                        Line.color.r         = 0.0;
                        Line.color.g         = 0.0;
                        Line.color.b         = 1.0;
                        Line.color.a         = 1.0;
