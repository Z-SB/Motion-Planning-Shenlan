function exp_array=expand_array(node_x,node_y,gn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y, hn_opt)
            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        ...
                        %exp_array(exp_count,3) = distance(xTarget,yTarget,s_x,s_y);% Euclidean distance,hn
                        exp_array(exp_count,3) = heuristic(xTarget,yTarget,s_x,s_y,hn_opt) % Diagonal Heuristic
