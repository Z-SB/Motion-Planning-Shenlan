...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    ROS_INFO("[node] receive the planning path");
    //Retrieve the path
...
int main(int argc, char** argv)
{
    ...
    nh.param("map/x_size",        _x_size, 50.0); //map_size(10,10,5)
    nh.param("map/y_size",        _y_size, 50.0);
    nh.param("map/z_size",        _z_size, 5.0 );
    
    nh.param("planning/start_x",  _start_pt(0),  0.0);  //start_pt(0,0,1)
    ...
    _max_x_id = (int)(_x_size * _inv_resolution); //(50,50,25)
