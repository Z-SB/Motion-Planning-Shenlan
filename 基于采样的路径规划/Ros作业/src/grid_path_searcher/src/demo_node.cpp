...
public:
    ...
    {   
        ...
        *
        *
        STEP 1: Extract the robot's (x,y,z) position from its state ； 将真实坐标从机器人状态空间中取出来并返回其free与否
        *
        *
        */
       double x = (*state3D)[0];
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    // Construct the robot state space in which we're planning. 创建机器人状态空间
    ob::StateSpacePtr space(new ob::RealVectorStateSpace(3));

    // Set the bounds of space to be in [0,1]. 设立空间界限
    ob::RealVectorBounds bounds(3);
    bounds.setLow(0, - _x_size * 0.5); // x, -5
    bounds.setLow(1, - _y_size * 0.5); // y, -5
    bounds.setLow(2, 0.0); // z, 0

    bounds.setHigh(0, + _x_size * 0.5); // x, 5
    bounds.setHigh(1, + _y_size * 0.5); // y, 5
    bounds.setHigh(2, _z_size); // z,2

    space->as<ob::RealVectorStateSpace>()->setBounds(bounds);

    // Construct a space information instance for this state space；为这个状态空间 创建空间信息实例
    ob::SpaceInformationPtr si(new ob::SpaceInformation(space));
    // Set the object used to check which states in the space are valid；在信息实例中储存各状态占用情况
    si->setStateValidityChecker(ob::StateValidityCheckerPtr(new ValidityChecker(si)));
    si->setup();

    // Set our robot's starting state 设定初始状态
   ...
   start->as<ob::RealVectorStateSpace::StateType>()->values[2] = start_pt(2);
    // Set our robot's goal state 设定目标状态
    ...
    *
    ...
    please define variable as pdef  创建问题实例
    * 
    *
    */
    ...
    // Set the optimization objective 设定优化目标
    /**
    *
    *
    ...
    *
    */  
   pdef->setOptimizationObjective(getPathLengthObjective(si)); //优化目标：最短路径
    ...
    *
    */ 
    ob::PlannerPtr optimizingPlanner(new og::RRTstar(si));  //最优化规划器：RRT star
    // Set the problem instance for our planner to solve  将问题实例输入规划器
    optimizingPlanner->setProblemDefinition(pdef);
    optimizingPlanner->setup();

    // attempt to solve the planning problem within one second of
    // planning time  试图1s中完成求解
    ...
    if (solved)
    {
        ...
        for (size_t path_idx = 0; path_idx < path->getStateCount (); path_idx++)
        {
            ...
            *
            *
            STEP 7: Trandform the found path from path to path_points for rviz display: 从状态空间中提取出路径点坐标，加入到路径队列中
            *
            *
            */ 
           Vector3d path_node;
           ...
           path_node(1) = (*state)[1];
           path_node(2) = (*state)[2];
           path_points.push_back(path_node);
        }
...
int main(int argc, char** argv)
{
    ...
    nh.param("map/x_size",        _x_size, 50.0); // 10 10 2
